...
class FlaskTestCase(unittest.TestCase):
    ...
    test_system_node = Node("System", system_id=system_id, system_uid=system_uid,
                            name=system_name, description="Unit Test Social System Description",
                            location_lat=42.338660, location_lng=-71.092186, status=0, creation_time=1461345863010,
    ...
    def test_search_systems(self, mocked):
        self.helper_create_user_node()
        self.helper_create_system_node()
        with self.app as client:
            with client.session_transaction() as session:
                session['uid'] = sql_id
        ...
        self.helper_delete_system_node()
        self.helper_delete_user_node()
    ...
    def helper_create_user_node(self):
        try:
    ...
    def helper_delete_user_node(self):
        try:
            delete_user_query = """
                MATCH (u:User)
                ...
                DETACH DELETE u
            """
            delete_user_status = graph.cypher.execute(delete_user_query, sql_id=sql_id)
        except Exception as ex:
            print "Exception At helper_delete_user_node: " + str(ex.message)

    # Helper Function To Create System Node In Neo4J Database
    def helper_create_system_node(self):
        try:
            # Pre Existing System Test Nodes Are Removed
            self.helper_delete_system_node()
            graph.create(test_system_node)
            print "helper create system"
        except Exception as ex:
    ...
    def helper_delete_system_node(self):
        try:
            delete_system_query = """
                MATCH (s:System)
                ...
                DETACH DELETE s
            """
            delete_system_status = graph.cypher.execute(delete_system_query, system_uid=system_uid)
            print "helper delete system"
        except Exception as ex:
